...
/*                                                                              */
/* DESCRIPTION                                                                  */
/*      Create cyclic handler(ID number automatic assignment)                   */
/*                                                                              */
/* INPUT                                                                        */
...
TX_INTERRUPT_SAVE_AREA
    ...
    if((cycid=_kernel_object_find(_kernel_cyc_id, _kernel_cyc_maxid))!=0)
    {
        if((rtn=(ER_ID)__cre_cyc(cycid, pk_ccyc))==E_OK)
        {
            rtn = (ER_ID)cycid;
        }
    }
